...
if (isset($_GET['bookingID']) && is_numeric($_GET['bookingID'])) {
    ...
    $checkSQL = "SELECT * FROM `flightbookings` WHERE bookingID='$bookingID' AND username='$user' AND cancelled='no'";
    $checkQuery = $conn->query($checkSQL);

    if ($checkQuery->num_rows > 0) {
        // Update the booking to mark it as canceled
        $cancelSQL = "UPDATE `flightbookings` SET cancelled='yes' WHERE bookingID='$bookingID'";
...
header("Location: userDashboardFlightBookings.php");
exit();
?>
